...
class TestOstoskori(unittest.TestCase):
    ...
    def test_kahden_eri_tuotteen_lisaamisen_jalkeen_korissa_kaksi_tavaraa(self):
        maito = Tuote("Maito", 3)
        peruna = Tuote("Peruna", 2)
        self.kori.lisaa_tuote(maito)
        self.kori.lisaa_tuote(peruna)

        self.assertEqual(self.kori.tavaroita_korissa(), 2)

    def test_kahden_eri_tuotteen_lisaamisen_jalkeen_ostoskorin_hinta_on_sama_kuintuotteiden_hintojen_summa(self):
        maito = Tuote("Maito", 3)
        peruna = Tuote("Peruna", 2)
        self.kori.lisaa_tuote(maito)
        self.kori.lisaa_tuote(peruna)
    ...
    def test_kahden_saman_tuotteen_lisaaamisen_jalkeen_ostoskori_sisaltaa_yhden_ostoksen(self):
        maito = Tuote("Maito", 3)
        self.kori.lisaa_tuote(maito)
        self.kori.lisaa_tuote(maito)
    ...
    def test_jos_korissa_on_kaksi_samaa_tuotetta_ja_toinen_poistetaan_koriin_jaa_ostos_jossa_on_tuotetta_yksi_kappale(self):
        maito = Tuote("Maito", 3)
        self.kori.lisaa_tuote(maito)
        self.kori.lisaa_tuote(maito)
